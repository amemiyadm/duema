...
class Turn:
    ...
    def charge_mana(self, hand_index):
        self.active_player.hand_to_mana_zone(hand_index)
        self.mana_charged = True

    def can_play_card(self, hand_index):
        card = self.active_player.hand[hand_index]
        if self.attacked:
    ...
    def can_use_mana(self, hand_index, mana_used):
        card = self.active_player.hand[hand_index]
    ...
    def play_card(self, hand_index, mana_used):
        for mana_index in mana_used:
        ...
        if card.card_type == 'creature':
            self.active_player.hand_to_battle_zone(hand_index)
        if card.card_type == 'spell':
            self.active_player.hand_to_graveyard(hand_index)

    def can_use_random(self, hand_index):
        card = self.active_player.hand[hand_index]
        mana_used = []
    ...
    def random_tap_play_card(self, hand_index, mana_used):
        card = self.active_player.hand[hand_index]
        for mana_index in mana_used:
            self.active_player.mana_zone[int(mana_index)].tap()
        self.card_played = True
    ...
    def play_shield_trigger(self, hand_index):
        card = self.inactive_player.hand[hand_index]
        for ability in card.get_on_play_abilities():
            if hasattr(ability, 'ability_conditions'):
                    if ability.ability_conditions == 'count':
                        self.stock.append(ability)
        ...
        if card.card_type == 'creature':
            self.inactive_player.hand_to_battle_zone(hand_index)
        if card.card_type == 'spell':
            self.inactive_player.hand_to_graveyard(hand_index)

    def can_attack_player(self, battle_zone_index):
        creature = self.active_player.battle_zone[battle_zone_index]
    ...
    def can_battle(self, battle_zone_index, attack_target_index):
        ...
        target_creature = self.inactive_player.battle_zone[attack_target_index]
    ...
    def battle(self, battle_zone_index, attack_target_index):
        ...
        if (attack_creature.power == target_creature.power):
            ...
            self.inactive_player.battle_zone_to_graveyard(attack_target_index)
    ...
    def block(self, battle_zone_index, blocker_index):
        attack_creature = self.active_player.battle_zone[battle_zone_index]
        target_creature = self.inactive_player.battle_zone[blocker_index]
        attack_creature.tap()
        target_creature.tap()
        self.attacked = True
        if (attack_creature.power == target_creature.power):
    ...
    def get_blockers(self, battle_zone_index):
        attacker = self.active_player.battle_zone[battle_zone_index]
